...
| B9 | Move the four paper agents (`research-lead`, `paper-author`, `manuscript-critic`, `bibliography`, in `~/.claude-personal/agents/`) off the `claude-opus-4-6` model pin onto the current family; `manuscript-humanizer` already pins `claude-sonnet-5` | Kerem declined the bump on 2026-09-22 as a second unrelated change in the flow update; `update-all-agents` exists for it | Kerem says, or the next edit to any of the four | 2026-09-22 |
| ~~B8~~ | ~~Replace the em-dashes in `paper/paper-odca-des.tex`~~ | closed 2026-09-20: 41 of them, not the 25 lines the row counted. Each replaced individually with a colon, comma, parentheses or a sentence split; en-dashes for ranges (S1--S4, request--seize--delay--release) left alone | closed | 2026-09-20 |
