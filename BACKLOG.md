...
| B7 | Decide whether the model should bound acceleration | neither ODCA-DES nor NaSch limits how fast a vehicle may speed up, so a vehicle leaving the posted zone in `fig:paradigm` returns to 5.2 cells/s in one cell entry rather than over a ramp; the figure now makes that visible and the Limitations list does not mention it | a referee raises it, or a scenario turns on start-up behaviour rather than steady flow | 2026-09-20 |
