...
Figure~\ref{fig:paradigm} shows what the two paradigms represent differently, on the same eight vehicles released from standstill three cells apart on the same road, both models deterministic so that only the representation differs. In panel~(a) the NaSch platoon exists only at whole seconds and only at whole cells: a vehicle is at cell~24 at $t = 3$\,s and at cell~27 at $t = 4$\,s, and the model says nothing about where it was in between, because there is no in between. In panel~(b) the same platoon under ODCA-DES has a mark wherever a vehicle entered a cell, and those instants are whatever the crossing times make them: of the 431 cell entries in the window, 11 fall on a whole second. The space axis is identical in both, which is the point of keeping the cell: ODCA-DES relaxes the time axis and the speed value, not the spatial discretisation that makes a CA cheap. Panel~(c) follows one vehicle out of the queue. The NaSch speed climbs through the integers 0, 1, 2, 3, 4, 5 and can hold no value between them; the ODCA-DES speed takes 29 distinct values between 0.36 and 5.20\,cells/s over the same fifteen seconds, and settles at the 5.2\,cells/s (140\,km/h) of Table~\ref{tab:vehicle_params}, a speed an integer-cell-per-second model cannot express at all: its nearest representable free-flow speeds are 5\,cells/s (135\,km/h) and 6\,cells/s (162\,km/h).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_paradigm_comparison.pdf}
    \caption{What the two paradigms represent, on the same eight vehicles released from standstill three cells apart, both models deterministic (no random slowdown, no driver heterogeneity). (a)~NaSch: a red mark is a position, and there is one per vehicle per whole second, each a whole number of cells ahead of the last. (b)~ODCA-DES: a red mark is a cell entry, and it occurs at whatever instant the crossing takes; 11 of the 431 entries shown fall on a whole second. The cell axis is the same in both. (c)~The speed of one vehicle leaving the queue: NaSch steps through the integers 0 to 5\,cells/s, while ODCA-DES passes through 29 distinct values between 0.36 and 5.20 and holds the 5.2\,cells/s that the integer model has no way to name. Produced by \texttt{code/plot\_paradigm\_comparison.py}.}
    \label{fig:paradigm}
\end{figure}
