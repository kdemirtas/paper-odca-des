...
%DIF ADD paper-odca-des.tex                 Sun Sep 20 12:42:19 2026
...
\begin{figure}[htbp]
    ...
    \caption{\DIFaddFL{What the two paradigms represent, on the same eight vehicles released from standstill three cells apart, both models deterministic (no random slowdown, no driver heterogeneity). (a)~NaSch: a red mark is a position, and there is one per vehicle per whole second, each a whole number of cells ahead of the last. (b)~ODCA-DES: a red mark is a cell entry, and it occurs at whatever instant the crossing takes; 11 of the 431 entries shown fall on a whole second. The cell axis is the same in both. (c)~The speed of one vehicle leaving the queue: NaSch steps through the integers 0 to 5\,cells/s, while ODCA-DES passes through 29 distinct values between 0.36 and 5.20 and holds the 5.2\,cells/s that the integer model has no way to name. Produced by }\texttt{\DIFaddFL{code/plot\_paradigm\_comparison.py}}\DIFaddFL{.}}
    \label{fig:paradigm}
\end{figure}
