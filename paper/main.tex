...
% \usepackage{algorithmic}  % TODO: enable when algorithm environment is needed
...
Under low demand (S1, 3000\,veh/h mainline), vehicles traverse the segment without significant resource contention. Figure~\ref{fig:tsd_freeflow} shows the resulting time-space diagram: trajectories are near-parallel lines with slope $\ell / v_f$, confirming that the event-driven movement process produces uniform free-flow speeds with only minor perturbations from HDV random slowdowns. Lane changes appear as discontinuities where a vehicle's trajectory shifts laterally.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_tsd_freeflow.pdf}
    \caption{Time-space diagram under free-flow conditions (single lane, 800\,veh/h). Vehicle trajectories are near-parallel with slope corresponding to $v_f = 5.2$\,cells/s ($\approx$140\,km/h). Minor speed fluctuations are due to HDV random slowdowns.}
    \label{fig:tsd_freeflow}
\end{figure}

\subsubsection{Congestion Formation and Wave Propagation}

Under high demand (S1, 6000\,veh/h mainline + ramp flows), congestion forms upstream of off-ramps where lane-changing vehicles create friction. As shown in Figure~\ref{fig:tsd_congested}, the time-space diagram reveals backward-propagating waves as regions where trajectory slopes decrease (lower speed) or become horizontal (stopped vehicles). The measured wave speed is consistent with the theoretical value $w = \ell/\tau$, demonstrating that congestion dynamics emerge naturally from the resource contention mechanism.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_tsd_congested.pdf}
    \caption{Time-space diagram under congested conditions (single lane, 2400\,veh/h). Trajectory slopes decrease as density increases beyond capacity, with backward-propagating waves visible as slope transitions. The measured wave speed is consistent with the theoretical value $w = \ell/\tau = 0.667$\,cells/s ($\approx$18\,km/h).}
    \label{fig:tsd_congested}
\end{figure}
...
Figure~\ref{fig:incident_trajectories} presents lane-by-lane time-space diagrams, where each line represents a vehicle trajectory colored by speed. Before the incident ($t < 300$\,s), trajectories are uniformly steep (high speed) across all lanes. Upon closure, vehicles in lane~4 decelerate sharply upstream of the incident zone, and mandatory lane changes---triggered early by the blockage-aware urgency inflation (Section~\ref{sec:lane_changing})---redistribute traffic to lanes~1--3. The resulting congestion wave propagates upstream, with the congestion front clearly visible as the boundary between green (free-flow) and yellow/red (congested) trajectory regions. The event-driven driver process ensures that vehicles respond immediately to the arriving congestion wave, while speed-dependent safety gaps (Eqs.~\ref{eq:front_gap}--\ref{eq:rear_gap}) facilitate merging without excessive disruption to adjacent lanes. After the blockage clears at $t = 1500$\,s, free-flow conditions gradually restore, with the queue fully dissipating by approximately $t \approx 3000$\,s.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_incident_trajectories.pdf}
    \caption{Lane-by-lane time-space diagrams during a 20-minute lane-closure incident. Trajectories are colored by speed (green = free flow, red = stopped). Dashed lines mark incident onset ($t=300$\,s, red) and clearance ($t=1500$\,s, blue). Lane~4 (leftmost) shows direct blockage impact with the closure zone visible as a vehicle-free gap; lanes~1--3 show progressive congestion from merging traffic, with recovery visible after clearance.}
    \label{fig:incident_trajectories}
\end{figure}

Figure~\ref{fig:incident_heatmap} provides the corresponding time-space density and speed heatmaps aggregated across all lanes. The density panel clearly shows the congestion region: elevated density expands upstream during the incident and gradually contracts after clearance. Backward-propagating stop-and-go waves are visible as diagonal bands of high density within the congested region. The speed panel mirrors this pattern, with low-speed regions upstream of the blockage and diagonal wave structures. The sharp boundary between congested and free-flow regimes is characteristic of kinematic wave theory, and its emergence from the resource-based simulation mechanics---without any macroscopic flow model---validates that the ODCA-DES framework faithfully reproduces first-order traffic dynamics at the macroscopic level while operating entirely at the microscopic, event-driven level. The slow recovery (approximately 25 minutes after clearance) reflects the hysteresis effect where the queue discharge rate initially falls below the arrival rate, consistent with empirical observations of incident recovery \citep{cassidy1998bivariate}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_incident_heatmap.pdf}
    \caption{Time-space heatmaps during the 20-minute lane-closure incident: (a)~density and (b)~space-mean speed, aggregated across all lanes with 15\,s $\times$ 75\,m bins. White/black dashed lines mark incident onset ($t=300$\,s) and clearance ($t=1500$\,s). The congestion region expands upstream during the closure with visible backward-propagating stop-and-go waves, then gradually contracts after clearance, with full recovery by approximately $t \approx 3000$\,s.}
