...
%DIF DEL /tmp/tmp.IFP2Q0BZus                Sun Sep 20 16:35:31 2026
%DIF ADD paper-odca-des-prerevision-2.tex   Sun Sep 20 03:44:19 2026
...
We derive closed-form expressions linking the resource protocol parameters to macroscopic traffic quantities---capacity, critical density, and backward wave speed---and extend the analysis to mixed AV/HDV traffic. Computational experiments on a four-lane freeway demonstrate that the framework reproduces smooth fundamental diagrams consistent with theory, captures transient congestion dynamics during lane-closure incidents, and models mixed autonomous and human-driven vehicle populations. \DIFdelbegin \DIFdel{Increasing }\DIFdelend \DIFaddbegin \DIFadd{In experiments with 20 independent replications, increasing }\DIFaddend AV penetration from 0\% to 70\% yields a \DIFdelbegin \DIFdel{54}\DIFdelend \DIFaddbegin \DIFadd{48}\DIFaddend \% throughput improvement \DIFaddbegin \DIFadd{(from $4{,}494 \pm 14$ to $6{,}664 \pm 48$\,veh/h, 95\% CI) }\DIFaddend and an 89\% delay reduction \DIFaddbegin \DIFadd{(from $254 \pm 7$ to $28 \pm 1$\,s)}\DIFaddend , driven by the reduced headway parameter in the resource protocol and the elimination of discretionary lane-change friction under centralized AV control. \DIFaddbegin \DIFadd{A lane-drop bottleneck experiment reveals that the throughput benefit of AV penetration plateaus near 50\% ($\approx$1}{\DIFadd{,}}\DIFadd{773\,veh/h), with the 50\% and 70\% AV scenarios producing statistically indistinguishable throughput.
...
\DIFaddbegin 
    ...
    \item \textbf{Physically motivated lane-change safety.} \DIFadd{Lane-change feasibility is assessed through speed-dependent closing-speed gaps, and mandatory lane changes incorporate blockage-aware urgency inflation, producing realistic incident dynamics without cooperative communication between vehicles.
}

    \item \textbf{Closed-form capacity expression for mixed traffic.} \DIFadd{The resource protocol parameters yield an analytical expression for the capacity effect of AV penetration, providing a direct link between simulation mechanics and macroscopic traffic quantities.
...
\begin{itemize}
    ...
    \item \textbf{Cumulative count} $N(t, x)$ can be constructed by counting arrivals at position $x$ up to time $t$, connecting to the LWR theory \citep{lighthill1955kinematic, richards1956shock} \DIFaddbegin \DIFadd{and its variational formulation \mbox{%DIFAUXCMD
\citep{daganzo2005variational}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
\end{itemize}
...
This triangular \DIFdelbegin \DIFdel{FD is }\DIFdelend \DIFaddbegin \DIFadd{FD---a two-regime piecewise-linear relationship first motivated by the empirical observations of \mbox{%DIFAUXCMD
\citet{greenshields1935study}}\hskip0pt%DIFAUXCMD
---is }\DIFaddend identical to that derived from Newell's kinematic wave model \citep{newell1993simplified} and the Cell Transmission Model \citep{daganzo1994cell}. The key insight is that the two parameters controlling the FD shape---$v_f$ (free-flow speed) and $w = \ell/\tau$ (backward wave speed)---emerge directly from the ODCA-DES model parameters: the cell length $\ell$ and the delayed release time $\tau$. No additional calibration of the FD is needed; it is an inherent consequence of the resource protocol.
...
\DIFdelbegin %DIFDELCMD < \label{sec:example}
%DIFDELCMD < %%%
\DIFdelend 
...
This section presents computational experiments designed to validate the analytical properties derived in Section~\ref{sec:analytical} and demonstrate the framework's capabilities for modeling heterogeneous multi-lane freeway traffic. All experiments are implemented using the ODCA-DES framework built on SimPy \citep{simpy2023} in Python\DIFaddbegin \DIFadd{~3.10, executed on a Linux workstation with an Intel Core i7-1255U (12 cores) and 32\,GB RAM. Wall-clock times reported throughout this section are measured as elapsed real time for the complete simulation run (excluding file I/O). Random number generation uses NumPy's }\texttt{\DIFadd{SeedSequence}} \DIFadd{to produce independent per-source streams}\DIFaddend .
...
Each scenario is run with \DIFdelbegin \DIFdel{10 replications using different random seeds }\DIFdelend \DIFaddbegin \DIFadd{20 independent replications (seeds 1--20) }\DIFaddend to account for stochastic variability in vehicle generation, type assignment, and HDV random slowdowns. \DIFaddbegin \DIFadd{All reported metrics are sample means with 95\% confidence intervals.
}\DIFaddend 
...
Under low demand (S1 \DIFdelbegin \DIFdel{, 3000}\DIFdelend \DIFaddbegin \DIFadd{parameters with reduced mainline demand of 3}{\DIFadd{,}}\DIFadd{000}\DIFaddend \,veh/h\DIFdelbegin \DIFdel{mainline}\DIFdelend ), vehicles traverse the segment without significant resource contention. Figure~\ref{fig:tsd}a shows the resulting time-space diagram: trajectories are near-parallel lines with slope $\ell / v_f$, confirming that the event-driven movement process produces uniform free-flow speeds with only minor perturbations from HDV random slowdowns.
...
Scenarios S1--S4 are compared across six performance metrics, measured over the full simulation period after a 300-second warm-up interval. All values are \DIFdelbegin \DIFdel{averaged over 10 replications }\DIFdelend \DIFaddbegin \DIFadd{means over 20 independent replications (seeds 1--20)}\DIFaddend ; 95\% confidence intervals are reported.
...
\begin{table}[ht]
\centering
\caption{Performance metrics across AV penetration scenarios ($T_{\text{sim}} = 3600$\,s, \DIFdelbeginFL \DIFdelFL{seed~42}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N = 20$ replications}\DIFaddendFL ). \DIFaddbeginFL \DIFaddFL{Values are means $\pm$ 95\% confidence intervals. }\DIFaddendFL The network operates near capacity with per-lane OD flows and three off-ramps requiring mandatory lane changes, creating congested baseline conditions.}
\label{tab:results_mixed}
\begin{tabular}{@{}lcccc@{}}
\toprule
Metric & S1 (0\%) & S2 (30\%) & S3 (50\%) & S4 (70\%) \\
\midrule
Throughput (veh/h) & \DIFdelbeginFL \DIFdelFL{4455 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$4{,}494 \pm 14$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{5374 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$5{,}416 \pm 24$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{6485 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$6{,}423 \pm 25$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{6845 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$6{,}664 \pm 48$ }\DIFaddendFL \\
Avg.\ travel time (s) & \DIFdelbeginFL \DIFdelFL{391.3 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$380.8 \pm 6.5$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{340.6 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$329.7 \pm 6.5$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{254.6 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$235.2 \pm 10.2$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{160.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$158.1 \pm 1.1$ }\DIFaddendFL \\
Avg.\ delay (s) & \DIFdelbeginFL \DIFdelFL{264.3 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$254.1 \pm 6.6$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{212.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$201.3 \pm 6.6$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{124.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$105.0 \pm 10.3$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{29.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$27.6 \pm 1.1$ }\DIFaddendFL \\
LC frequency (/veh/km) & \DIFdelbeginFL \DIFdelFL{4.90 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$4.89 \pm 0.02$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{3.54 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$3.49 \pm 0.02$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2.27 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2.23 \pm 0.04$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.19 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.22 \pm 0.01$ }\DIFaddendFL \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Throughput.} Network throughput increases monotonically with AV penetration, from \DIFdelbegin \DIFdel{4}%DIFDELCMD < {%%%
\DIFdel{,}%DIFDELCMD < }%%%
\DIFdel{455}\DIFdelend \DIFaddbegin \DIFadd{$4{,}494 \pm 14$}\DIFaddend \,veh/h at $\alpha = 0\%$ to \DIFdelbegin \DIFdel{6}%DIFDELCMD < {%%%
\DIFdel{,}%DIFDELCMD < }%%%
\DIFdel{845}\DIFdelend \DIFaddbegin \DIFadd{$6{,}664 \pm 48$}\DIFaddend \,veh/h at $\alpha = 70\%$, a \DIFdelbegin \DIFdel{54}\DIFdelend \DIFaddbegin \DIFadd{48}\DIFaddend \% improvement. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The confidence intervals are tight (coefficient of variation $< 1.6\%$ in all scenarios), confirming that 20 replications provide sufficient statistical resolution. The improvement }\DIFaddend is below the theoretical 71\% capacity increase (Table~\ref{tab:theoretical_capacity}), which assumes homogeneous mixed traffic without lane-change friction. The gap is attributable to ramp interactions and mandatory lane changes---vehicles destined for off-ramps must merge to the rightmost lane, creating cross-lane friction irrespective of vehicle type. \DIFaddbegin \DIFadd{Figure~\ref{fig:throughput_bar} visualizes the throughput and delay trends with 95\% confidence intervals.
}\DIFaddend 

\paragraph{Delay.} Average delay drops from \DIFdelbegin \DIFdel{264}\DIFdelend \DIFaddbegin \DIFadd{$254.1 \pm 6.6$}\DIFaddend \,s to \DIFdelbegin \DIFdel{29}\DIFdelend \DIFaddbegin \DIFadd{$27.6 \pm 1.1$}\DIFaddend \,s as AV penetration increases from 0\% to 70\%, an 89\% reduction. The high baseline delay reflects the congested operating conditions: the per-lane OD flows with destination distributions require substantial lane-changing activity, and the combined mainline and ramp demand (7{,}000\,veh/h) loads the network near its effective capacity. The event-driven driver process enables vehicles to respond immediately to changing gaps rather than waiting for fixed evaluation intervals, and at 70\% AV penetration the shorter headway ($\tau = 0.5$\,s vs.\ 1.5\,s) significantly reduces car-following delays throughout the network. \DIFaddbegin \DIFadd{Figure~\ref{fig:travel_time} presents the average travel time across scenarios.
}\DIFaddend 

\paragraph{Lane-change frequency.} Lane-change frequency \DIFdelbegin \DIFdel{per vehicle per km }\DIFdelend \emph{decreases} with AV penetration \DIFdelbegin \DIFdel{, from 4.90\,LC/km (S1) to 1.19\,LC/km (S4}\DIFdelend \DIFaddbegin \DIFadd{(Table~\ref{tab:results_mixed}}\DIFaddend ). Because AVs perform only mandatory lane changes, increasing AV penetration eliminates discretionary lane-change friction. The remaining lane changes are driven entirely by route requirements (off-ramp exits), and AVs' shorter headways ($\tau = 0.5$\,s) create more available gaps for these mandatory maneuvers, reducing queuing behind slower vehicles in the target lane.

\DIFaddbegin \begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_throughput_bar.pdf}
    \caption{\DIFaddFL{Network throughput and delay across AV penetration scenarios (S1--S4, $N = 20$ replications). Error bars denote 95\% confidence intervals. Throughput increases monotonically from $4{,}494$ to $6{,}664$\,veh/h, while delay decreases from 254 to 28\,s.}}
...
\begin{figure}[ht]
    ...
    \caption{\DIFaddFL{Average travel time across AV penetration scenarios (S1--S4, $N = 20$ replications). Error bars denote 95\% confidence intervals. Travel time decreases from 381\,s (S1) to 158\,s (S4), approaching the full-segment free-flow travel time of 154\,s ($= 800 \cdot 7.5\,\text{m}\;/\;(5.2 \times 7.5\,\text{m/s})$).}}
    \label{fig:travel_time}
\end{figure}

\DIFaddend \paragraph{Computational note.} Wall-clock times range from 277\,s (S4) to 802\,s (S1) for 3{,}600\,s of simulated time. The all-HDV baseline is slowest because heavy congestion generates the most resource-contention events. As AV penetration increases, reduced lane-change friction and shorter headways alleviate congestion, decreasing the total event count despite the AV controller's 10\,Hz evaluation frequency.



\DIFaddbegin \subsection{\DIFadd{Bottleneck Analysis: Lane-Drop Scenario}}
\label{sec:exp_bottleneck}

\DIFadd{To evaluate the framework's performance under capacity-reducing geometry, we simulate a 3-lane, 6\,km freeway segment where the rightmost lane drops at the midpoint, forcing all lane-1 traffic to merge into lanes~2--3. Demand is 3}{\DIFadd{,}}\DIFadd{600\,veh/h (1}{\DIFadd{,}}\DIFadd{200\,veh/h/lane), and AV penetration is varied across 0\%, 30\%, 50\%, and 70\%. Each scenario is run with 20 independent replications.
}

\DIFadd{Table~\ref{tab:results_bottleneck} summarizes the results.
}

\begin{table}[ht]
\centering
\caption{\DIFaddFL{Bottleneck performance metrics ($N = 20$ replications). Values are means $\pm$ 95\% confidence intervals. BN$_\alpha$ denotes the bottleneck scenario at AV penetration $\alpha$.}}
\label{tab:results_bottleneck}
\begin{tabular}{@{}lcccc@{}}
\toprule
\DIFaddFL{Metric }& \DIFaddFL{BN$_{0\%}$ }& \DIFaddFL{BN$_{30\%}$ }& \DIFaddFL{BN$_{50\%}$ }& \DIFaddFL{BN$_{70\%}$ }\\
\midrule
\DIFaddFL{Throughput (veh/h) }& \DIFaddFL{$1{,}341 \pm 111$ }& \DIFaddFL{$1{,}569 \pm 99$ }& \DIFaddFL{$1{,}773 \pm 164$ }& \DIFaddFL{$1{,}780 \pm 174$ }\\
\DIFaddFL{Avg.\ delay (s) }& \DIFaddFL{$318.1 \pm 32.0$ }& \DIFaddFL{$320.3 \pm 43.2$ }& \DIFaddFL{$214.7 \pm 42.9$ }& \DIFaddFL{$142.3 \pm 42.6$ }\\
\DIFaddFL{LC frequency (/veh/km) }& \DIFaddFL{$2.11 \pm 0.26$ }& \DIFaddFL{$1.59 \pm 0.14$ }& \DIFaddFL{$1.22 \pm 0.14$ }& \DIFaddFL{$0.74 \pm 0.07$ }\\
\bottomrule
\end{tabular}
\end{table}

\paragraph{\DIFadd{Throughput plateau near 50\% AV penetration.}}
\DIFadd{Unlike the open-freeway scenarios (S1--S4), where throughput increases monotonically across the full penetration range, the bottleneck exhibits a throughput plateau above 50\% AV penetration: BN$_{50\%}$ achieves $1{,}773 \pm 164$\,veh/h while BN$_{70\%}$ yields $1{,}780 \pm 174$\,veh/h---a difference of 8\,veh/h with fully overlapping 95\% confidence intervals (Figure~\ref{fig:bottleneck_throughput}). The two scenarios are statistically indistinguishable.
}

\DIFadd{Note that per-vehicle delay at BN$_{30\%}$ ($320.3 \pm 43.2$\,s) is virtually identical to BN$_{0\%}$ ($318.1 \pm 32.0$\,s) despite the 17\% throughput increase; the higher throughput means that more vehicles complete long-delay trips that would have been blocked entirely at BN$_{0\%}$, holding the per-vehicle average delay roughly constant even as aggregate performance improves. The confidence intervals also overlap substantially.
}

\DIFadd{The mechanism underlying the throughput plateau is merge-capacity saturation: at 50\% AV penetration, the smoother headways of AVs ($\tau = 0.5$\,s vs.\ 1.5\,s for HDVs) already enable near-maximum utilization of the two remaining lanes at the merge point. Additional AVs beyond this threshold reduce per-vehicle delay (from $214.7 \pm 42.9$\,s to $142.3 \pm 42.6$\,s) by improving upstream flow conditions, but the downstream capacity at the 2-lane bottleneck throat cannot absorb further throughput gains. This finding is qualitatively consistent with the concavity of the theoretical capacity function $q_{\max}(\alpha)$ (Eq.~\ref{eq:mixed_capacity}), in which the marginal headway reduction per additional AV diminishes with increasing $\alpha$; however, the plateau itself reflects merge dynamics at the lane drop that are not captured by the idealized single-stream capacity formula.
}

\DIFadd{The wider confidence intervals in the bottleneck scenarios (coefficient of variation 10--21\%, compared to 0.7--1.6\% for S1--S4) reflect the sensitivity of merge dynamics to random vehicle sequencing at the lane drop. The stochastic order in which HDVs and AVs arrive at the merge point creates higher run-to-run variability than the distributed ramp interactions of the open-freeway network.
}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig_bottleneck_throughput.pdf}
    \caption{\DIFaddFL{Bottleneck throughput across AV penetration levels ($N = 20$ replications). Error bars denote 95\% confidence intervals. The throughput benefit plateaus near 50\% AV penetration: the BN$_{50\%}$ and BN$_{70\%}$ confidence intervals overlap almost entirely, indicating no statistically significant throughput gain beyond 50\% in a capacity-constrained merge.}}
    \label{fig:bottleneck_throughput}
\end{figure}


\subsection{\DIFadd{Sensitivity to HDV Action Interval}}
\label{sec:exp_sensitivity}

\DIFadd{The HDV action interval $\delta_{\text{eval}}$ (the maximum time between driver re-evaluations; Section~\ref{sec:driver}) is a key model parameter that controls how frequently human drivers reassess speed and lane-change decisions. The default value of $\delta_{\text{eval}} = 1.0$\,s reflects typical human perception-reaction cycling, but lower values approximate more attentive driving or a finer-grained discrete-time approximation of continuous decision-making.
}

\DIFadd{To quantify the sensitivity of simulation outcomes to this parameter, we run the S1 (all-HDV) scenario at three action-interval values: $\delta_{\text{eval}} \in \{0.25, 0.5, 1.0\}$\,s, with 10 replications at $\delta_{\text{eval}} = 0.25$ and $0.5$\,s and 20 replications at $\delta_{\text{eval}} = 1.0$\,s (the baseline from Section~\ref{sec:exp_mixed}). Table~\ref{tab:sensitivity_ai} reports the results; Figure~\ref{fig:sensitivity_ai} presents the trends graphically.
}

\begin{table}[ht]
\centering
\caption{\DIFaddFL{Sensitivity of S1 (0\% AV) performance to HDV action interval. Values are means $\pm$ 95\% CI.}}
\label{tab:sensitivity_ai}
\begin{tabular}{@{}lccc@{}}
\toprule
\DIFaddFL{Metric }& \DIFaddFL{$\delta_{\text{eval}} = 0.25$\,s }& \DIFaddFL{$\delta_{\text{eval}} = 0.5$\,s }& \DIFaddFL{$\delta_{\text{eval}} = 1.0$\,s }\\
\midrule
\DIFaddFL{Throughput (veh/h) }& \DIFaddFL{$4{,}870 \pm 16$ }& \DIFaddFL{$4{,}762 \pm 23$ }& \DIFaddFL{$4{,}494 \pm 14$ }\\
\DIFaddFL{Avg.\ travel time (s) }& \DIFaddFL{$330.2 \pm 4.1$ }& \DIFaddFL{$346.4 \pm 8.6$ }& \DIFaddFL{$380.8 \pm 6.5$ }\\
\DIFaddFL{Avg.\ delay (s) }& \DIFaddFL{$202.6 \pm 4.2$ }& \DIFaddFL{$219.0 \pm 8.6$ }& \DIFaddFL{$254.1 \pm 6.6$ }\\
\DIFaddFL{LC frequency (/veh/km) }& \DIFaddFL{$4.10 \pm 0.02$ }& \DIFaddFL{$4.39 \pm 0.03$ }& \DIFaddFL{$4.89 \pm 0.02$ }\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
    ...
    \caption{\DIFaddFL{Sensitivity of S1 performance metrics to HDV action interval $\delta_{\text{eval}}$. Panels show throughput, average travel time, average delay, and lane-change frequency. Error bars denote 95\% confidence intervals. Reducing the action interval monotonically improves all metrics.}}
    \label{fig:sensitivity_ai}
\end{figure}

\DIFadd{Reducing the action interval from 1.0\,s to 0.25\,s increases throughput by 8.4\% (from $4{,}494 \pm 14$ to $4{,}870 \pm 16$\,veh/h) and reduces average delay by 20\% (from $254 \pm 7$ to $203 \pm 4$\,s). The effect is monotonic across all metrics: finer-grained driver re-evaluation enables smoother speed adjustments and earlier gap exploitation, reducing the stop-and-go oscillations that arise when drivers react to stale state information. Lane-change frequency decreases at lower action intervals, indicating that more responsive drivers maintain better longitudinal flow, reducing the need for discretionary lane changes.
}

\DIFadd{This result has a structural interpretation: as $\delta_{\text{eval}} \to 0$, the event-driven driver process converges toward the continuous-time limit of the Newell car-following model, closing the gap between the discrete-event implementation and the underlying continuous-time theory. The 8.4\% throughput improvement from a 4$\times$ reduction in action interval quantifies this structural gap, suggesting that the default 1.0\,s value is a conservative but reasonable operating point.
}

\paragraph{\DIFadd{Scope note.}} \DIFadd{This sensitivity analysis is restricted to S1 (all-HDV) because the action interval is an HDV parameter; AVs operate on a fixed 10\,Hz controller cycle irrespective of $\delta_{\text{eval}}$. In mixed-traffic scenarios (S2--S4), HDVs constitute a decreasing fraction of the fleet, so the sensitivity to $\delta_{\text{eval}}$ is proportionally smaller. Additionally, reducing $\delta_{\text{eval}}$ in mixed-traffic scenarios with high event density proved computationally intractable within the experimental budget. Note that the $\delta_{\text{eval}} = 0.25$ and $0.5$\,s scenarios use $N = 10$ replications versus $N = 20$ for the baseline at $\delta_{\text{eval}} = 1.0$\,s; accordingly, the 95\% confidence intervals at the two lower values are computed with fewer degrees of freedom and are somewhat wider than they would be at $N = 20$.
}


\DIFaddend \subsection{Computational Performance}
\DIFdelbegin %DIFDELCMD < \label{sec:exp_performance}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:exp_performance_new}

\subsubsection{\DIFadd{Scalability with Network Size}}
\label{sec:exp_scalability}

\DIFadd{To assess how the event-driven architecture scales with network size, we simulate the S1 (all-HDV) scenario on networks of increasing size: 200, 400, 800, 1}{\DIFadd{,}}\DIFadd{600, and 3}{\DIFadd{,}}\DIFadd{200 cells per lane (4 lanes each, yielding 800 to 12}{\DIFadd{,}}\DIFadd{800 total cells), with 3 seeds per configuration. All networks use the same demand density and simulation duration ($T_{\text{sim}} = 1{,}800$\,s). Table~\ref{tab:scalability} reports the results; Figure~\ref{fig:scalability} presents the scaling trends.
}

\begin{table}[ht]
\centering
\caption{\DIFaddFL{Computational scalability across network sizes (S1, all-HDV, $N = 3$ seeds per size, $T_{\text{sim}} = 1{,}800$\,s). Total cells = num\_cells $\times$ 4 lanes.}}
\label{tab:scalability}
\begin{tabular}{@{}rrrrl@{}}
\toprule
\DIFaddFL{Cells/lane }& \DIFaddFL{Total cells }& \DIFaddFL{Events ($\times 10^3$) }& \DIFaddFL{Wall clock (s) }& \DIFaddFL{Realtime ratio }\\
\midrule
\DIFaddFL{200 }& \DIFaddFL{800 }& \DIFaddFL{394 }& \DIFaddFL{227 }& \DIFaddFL{$8.0\times$ }\\
\DIFaddFL{400 }& \DIFaddFL{1}{\DIFaddFL{,}}\DIFaddFL{600 }& \DIFaddFL{619 }& \DIFaddFL{370 }& \DIFaddFL{$5.2\times$ }\\
\DIFaddFL{800 }& \DIFaddFL{3}{\DIFaddFL{,}}\DIFaddFL{200 }& \DIFaddFL{910 }& \DIFaddFL{448 }& \DIFaddFL{$4.1\times$ }\\
\DIFaddFL{1}{\DIFaddFL{,}}\DIFaddFL{600 }& \DIFaddFL{6}{\DIFaddFL{,}}\DIFaddFL{400 }& \DIFaddFL{1}{\DIFaddFL{,}}\DIFaddFL{364 }& \DIFaddFL{786 }& \DIFaddFL{$2.3\times$ }\\
\DIFaddFL{3}{\DIFaddFL{,}}\DIFaddFL{200 }& \DIFaddFL{12}{\DIFaddFL{,}}\DIFaddFL{800 }& \DIFaddFL{1}{\DIFaddFL{,}}\DIFaddFL{935 }& \DIFaddFL{1}{\DIFaddFL{,}}\DIFaddFL{533 }& \DIFaddFL{$1.2\times$ }\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
    ...
    \caption{\DIFaddFL{Computational scalability of ODCA-DES. Left: total events and wall-clock time vs.\ network size (log--log scale). Right: realtime ratio vs.\ total cells. Event count scales sub-linearly with network size, reflecting event locality in the DES architecture.}}
    \label{fig:scalability}
\end{figure}

\DIFadd{The total event count increases by a factor of 4.9$\times$ for a 16$\times$ increase in cell count (200 to 3}{\DIFadd{,}}\DIFadd{200 cells per lane), confirming sub-linear scaling (empirical exponent $\approx 0.57$, i.e., $O(n^{0.57})$). This is a direct consequence of event locality: events are generated by vehicle interactions, not by grid traversal, so expanding the network with proportional demand increases the number of interacting vehicle pairs sub-linearly as congestion remains localized. Wall-clock time increases by 6.8$\times$ over the same range, reflecting both the event count growth and the increased overhead of managing a larger event calendar. The realtime ratio decreases from $8.0\times$ (800 total cells) to $1.2\times$ on average (12}{\DIFadd{,}}\DIFadd{800 total cells), maintaining near-realtime or faster execution across the tested network sizes.
}


\subsubsection{\DIFadd{Event Count Analysis}}
\label{sec:exp_event_count}
\DIFaddend 

To assess the computational characteristics of the event-driven approach \DIFaddbegin \DIFadd{on a per-scenario basis}\DIFaddend , we compare the number of simulation events and wall-clock execution time against a timestep-equivalent simulation on the same \DIFdelbegin \DIFdel{network and demand}\DIFdelend \DIFaddbegin \DIFadd{4-lane, 800-cell network}\DIFaddend .

\DIFdelbegin \subsubsection{\DIFdel{Event Count Analysis}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In a timestep-based simulator with update interval $\Delta t$, the number of update operations is:
\begin{equation}
...
Table~\ref{tab:computational} compares the event counts and wall-clock times across \DIFaddbegin \DIFadd{the S1--S4 }\DIFaddend scenarios.
...
CF evaluations ($\times 10^6$) & 2.09 & 6.87 & 9.24 & 7.13 \\
Lane changes & 126{,}773 & 105{,}959 & 80{,}206 & 42{,}334 \\
Wall-clock time --- ODCA-DES (s) & 802 & 477 & 318 & 277 \\
\bottomrule
\end{tabular}
\end{table}

For the all-HDV baseline (S1), car-following evaluations total 2.09~million---three times fewer than the 6.4~million timestep updates that would be required by a comparable time-stepped model with $\Delta t = 0.25$\,s. The event-driven driver process evaluates each vehicle only when neighbors move or the action interval expires, not at every global timestep. As AV penetration increases, CF evaluations rise due to the central AV controller's 10\,Hz evaluation frequency and the event-driven wake-up mechanism, peaking at 9.2~million at 50\% AV before declining to 7.1~million at 70\%---as AV dominance reduces lane-change friction and the associated cascading re-evaluations. Lane-change frequency \emph{decreases} monotonically with AV penetration (from 126{,}773 to 42{,}334), since AVs perform only mandatory lane changes.

Wall-clock times range from 277\,s (S4) to 802\,s (S1), yielding real-time ratios of 4.5--13$\times$. The key computational advantage of ODCA-DES is the \emph{locality} of events: each event involves only one vehicle and its immediate neighbors, avoiding the global synchronization required by timestep updates. The event-driven driver process amplifies this advantage---vehicles in free-flow regions with no nearby neighbors generate only periodic re-evaluations, while computational effort concentrates at congested locations where interactions occur. This locality enables natural parallelization and makes the computational cost proportional to the \emph{interacting} vehicle count rather than the total vehicle count---an advantage that grows with network size and free-flow fraction.
...
Computational experiments \DIFaddbegin \DIFadd{with 20 independent replications }\DIFaddend demonstrated the framework's ability to reproduce expected traffic dynamics across multiple scenarios. The fundamental diagram validation confirmed close agreement with the theoretical triangular FD, with scatter patterns arising from per-driver heterogeneity. Under increasing AV penetration from 0\% to 70\%, network throughput increased by 48\% (from \DIFdelbegin \DIFdel{4}%DIFDELCMD < {%%%
\DIFdel{,}%DIFDELCMD < }%%%
\DIFdel{419 to 6}%DIFDELCMD < {%%%
\DIFdel{,}%DIFDELCMD < }%%%
\DIFdel{531}\DIFdelend \DIFaddbegin \DIFadd{$4{,}494 \pm 14$ to $6{,}664 \pm 48$}\DIFaddend \,veh/h) and average delay decreased \DIFdelbegin \DIFdel{from 266\,s to 111}\DIFdelend \DIFaddbegin \DIFadd{by 89\% (from $254 \pm 7$ to $28 \pm 1$}\DIFaddend \,s\DIFaddbegin \DIFadd{) }\DIFaddend on a congested 4-lane network with per-lane origin--destination flows. \DIFaddbegin \DIFadd{A lane-drop bottleneck experiment revealed that the throughput benefit of AV penetration plateaus near 50\%, with the 50\% and 70\% AV scenarios producing statistically indistinguishable throughput ($1{,}773 \pm 164$ vs.\ $1{,}780 \pm 174$\,veh/h)---a finding attributable to merge-capacity saturation at the 2-lane bottleneck throat. Sensitivity analysis showed that reducing the HDV action interval from 1.0\,s to 0.25\,s increases throughput by 8.4\%, quantifying the structural gap between the discrete-event implementation and the continuous-time theoretical limit. }\DIFaddend The temporary incident experiment showed that backward-propagating stop-and-go waves, congestion spillback, and gradual recovery emerge naturally from the event-driven resource protocol.

The event-driven architecture achieved real-time ratios of \DIFdelbegin \DIFdel{9--12$\times$ on a standard workstation}\DIFdelend \DIFaddbegin \DIFadd{$1.2$--$8.0\times$ on a single-threaded Python process (hardware as specified in Section~\ref{sec:exp_setup}), with sub-linear event scaling across network sizes from 800 to 12}{\DIFadd{,}}\DIFadd{800 cells}\DIFaddend . The DES paradigm's inherent event locality---where computational effort concentrates at congested locations and times---provides a foundation for future parallelization and large-network scalability that fixed-timestep approaches cannot match without significant algorithmic modification.
...
\begin{itemize}
    \item \DIFaddbegin \textbf{No empirical calibration.} \DIFadd{All model parameters (reaction times, headway distributions, lane-change thresholds) are derived from representative values in the literature rather than calibrated to a specific field dataset. Validation against empirical trajectory data---such as NGSIM or highD---would require matching simulated and observed fundamental diagrams, headway distributions, and lane-change rates at specific sites. Until such calibration is performed, the quantitative predictions (absolute throughput, delay magnitudes) should be interpreted as framework demonstrations rather than site-specific forecasts.
}

    \item \textbf{Single unidirectional freeway segment.} \DIFadd{The test network is a single straight freeway with on/off-ramps. There are no merge/diverge junctions, weaving sections, signalized ramp meters, or route-choice decisions. The resource-based architecture is in principle extensible to such geometries, but the current implementation does not support them.
}

    \item \textbf{Static demand.} \DIFadd{All experiments use constant origin--destination flow rates. The event-driven architecture is compatible with time-varying demand---injection rates can be scheduled as time-dependent events---but the current implementation does not expose this capability. Consequently, the framework has not been tested under peak-hour demand ramps, demand shocks, or diurnal patterns.
}

    \item \DIFaddend \textbf{Limited heterogeneity calibration.} While per-driver parameter sampling (LogNormal for $\tau$ and action interval, Normal for slowdown probability) introduces within-type heterogeneity, the distribution parameters are \DIFdelbegin \DIFdel{based on representative values rather than calibrated to empirical distributions of driver behavior. The lane-change }\DIFdelend \DIFaddbegin \DIFadd{literature-derived rather than fitted to observed driver populations. Lane-change }\DIFaddend parameters remain homogeneous within each vehicle type.

    \item \DIFdelbegin %DIFDELCMD < \textbf{Static demand.} %%%
\DIFdel{All experiments use fixed origin--destination flow rates. Time-varying and stochastic demand patterns, which are critical for modeling peak-hour dynamics and incident response, are not considered.
}%DIFDELCMD < 

%DIFDELCMD <     \item \textbf{No empirical calibration.} %%%
\item%DIFAUXCMD
\DIFdel{The model parameters are based on representative values from the literature rather than calibrated to a specific field dataset. Validation against empirical trajectory data (e.g., NGSIM, highD) would strengthen confidence in the framework's quantitative predictions. }%DIFDELCMD < 

%DIFDELCMD <     \item \textbf{Single freeway segment.} %%%
\item%DIFAUXCMD
\DIFdel{The network is a single unidirectional freeway with on /off-ramps. Extension to freeway networks with interchanges, weaving sections, and corridor-level route choice remains future work}\DIFdelend \DIFaddbegin \textbf{Sensitivity scope.} \DIFadd{The action-interval sensitivity analysis (Section~\ref{sec:exp_sensitivity}) covers only the all-HDV scenario (S1). Because the action interval is an HDV parameter and AVs operate on a fixed 10\,Hz controller cycle, the sensitivity is greatest in HDV-dominated traffic. At higher AV penetration levels, HDV behavioral parameters have proportionally less influence on aggregate performance. Extending the sensitivity analysis to mixed-traffic scenarios at low action intervals was computationally intractable within the experimental budget}\DIFaddend .
...
\begin{itemize}
    \item \DIFdelbegin %DIFDELCMD < \textbf{Calibrated driver heterogeneity.} %%%
\DIFdel{The current per-driver parameter sampling uses representative distribution parameters; calibrating these distributions against empirical trajectory datasets would produce more realistic traffic dynamics and enable the study of behavioral variability effects on traffic stability.
}%DIFDELCMD < 

%DIFDELCMD <     \item %%%
\item%DIFAUXCMD
\DIFdelend \textbf{Empirical calibration and validation.} The $T(x,n)$ trajectory output format is well-suited for direct comparison with trajectory datasets such as NGSIM and highD, which provide vehicle-level passage times at discrete cross-sections. \DIFaddbegin \DIFadd{A systematic calibration study would fit the reaction-time, headway, and lane-change distributions to observed trajectory data at specific sites, enabling quantitative validation of the fundamental diagram shape, headway distributions, and congestion wave speeds.
}\DIFaddend 

    \item \textbf{Network-level extension.} The cell-based resource model can be extended to freeway networks by defining merge and diverge cells with multi-input or multi-output resource connections, enabling corridor-level simulations \DIFaddbegin \DIFadd{with interchanges, weaving sections, and route-choice decisions.
}

    \item \textbf{Time-varying demand.} \DIFadd{The event-driven architecture is compatible with time-varying injection rates scheduled as time-dependent events. Implementing and validating this capability would enable simulation of peak-hour demand ramps, demand shocks, and diurnal traffic patterns.
}

    \item \textbf{Expanded sensitivity analysis.} \DIFadd{Extending the action-interval sensitivity to mixed-traffic scenarios (S2--S4) would quantify the interaction between HDV behavioral granularity and AV penetration level. Computational advances (e.g., event batching or parallel DES engines) may be needed to make low-action-interval mixed-traffic runs tractable.
}

    \item \textbf{Calibrated driver heterogeneity.} \DIFadd{Calibrating the per-driver parameter distributions against empirical driver-behavior data would produce more realistic traffic dynamics and enable the study of behavioral variability effects on traffic stability and capacity}\DIFaddend .
