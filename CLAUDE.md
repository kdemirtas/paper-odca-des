...
- **Build:** pdflatex, bibtex, pdflatex, pdflatex from `paper/` with `-interaction=nonstopmode`,
  on all three journal files; done means 0 errors, 0 undefined references, 0 missing citations, 0
  natbib "Author undefined" warnings (a numbered style without author labels prints "(author?)"
  for every `\citet`, D-2026-09-22-3), no overfull hbox over 10 pt, in each of them.
...
- **One manuscript file per target journal** (D-2026-09-20-14, D-2026-09-22-2): three files.
  `paper/paper-odca-des_treng.tex` (Transportation Engineering, the target) is `_trb` with four
  differences: the `\journal` line, the `number` class option, the competing-interest declaration in
  the journal's template for editor-affiliated authors, and `\bibliographystyle{elsarticle-num-names}`
  (numbered references with author labels, D-2026-09-22-3). `diff` of `_treng` against `_trb`
  shows four hunks, against `_smpt` nine. TRENG also submits `paper/highlights_treng.txt`.
  ...
  `diff paper-odca-des_treng.tex paper-odca-des_trb.tex` four; one more in either means a shared
