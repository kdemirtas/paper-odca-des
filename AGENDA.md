...
| **Transportation Engineering** (chosen 2026-09-22, D-2026-09-22-2) | 7.9, SJR 0.949, no impact factor (Scopus, DOAJ; not SCIE) | Q1 in Scopus engineering categories | not measured | about 12 weeks to publication | not measured | strong: applied transportation engineering, traffic simulation and AVs in scope; gold OA, APC up to USD 2,310; a co-author is an Associate Editor there (declared in the TRENG file, recused); APC waiver for open-science components reported by Zhou, unverified |

  **Closed 2026-09-22: Kerem chose Transportation Engineering** on his co-advisor's information (D-2026-09-22-2); `paper/paper-odca-des_treng.tex` is the file to send. The comparison below stands as the record of the earlier analysis.

  **Earlier recommendation: Simulation Modelling Practice and Theory**, with Transportation Research Part C as the aspirational alternative and Physica A as the fallback. On Kerem's three criteria it is the only one that wins all three: a CiteScore of 9.9 against TR-B's 10.7, so the index barely moves; about 3 months of review against TR-B's 6.3; and a competitiveness LetPub rates easy against TR-B's 131 slots a year. The scope match is the real argument: this paper's contribution is a simulation paradigm (discrete-event mechanics on a cellular automaton), which is that journal's subject, whereas at TR-B it competes against traffic-flow theory papers on their own ground.
  ...
  **Both versions are now written (2026-09-20, D-2026-09-20-14), so this is a choice of file, not more work.** `paper/paper-odca-des_trb.tex` (45 pages) and `paper/paper-odca-des_smpt.tex` (46 pages) differ in six places: the `\journal` line, the abstract opening, the introduction's first paragraph, the generality sentences closing the introduction, the generality sentences closing the conclusion, and a Future Research bullet on transfer to a second domain. Everything else is shared and both build clean.

  **One fact found while doing it, which bears on the choice.** The DEVS literature search (D-2026-09-20-13) turned up that a cellular automaton on a discrete-event engine is established since Cell-DEVS in 2001, and that ATLAS, the closest prior work, was published in Simulation Modelling Practice and Theory itself. That cuts both ways: an SMPT referee is more likely than a TR-B one to know this literature, which raises the bar there, but it also means the paper has now been rewritten to claim only the inversion (passive cell, vehicle as process) rather than the combination. It would have been exposed at either venue; it is exposed at neither now.
