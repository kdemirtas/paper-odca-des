...
**RESUME:** N11 is done: the rerun is in `code/output/` and every quoted number is restated. Next is Kerem's read of `paper/paper-odca-des.pdf` with `paper/revision-2.diff`, then the critic round; this repo's assumptions are all closed, the DLC rate question is still open in odca-des.
...
2. **Close the odca-des assumptions** with `/next-assumption` there (17 rows, the discretionary lane-change rate among them). This repo's three closed on 2026-09-20: the wall-time definition (D-2026-09-20-2), the numbered revision files (D-2026-09-20-3) and the incident scenario (D-2026-09-20-4); `ASSUMPTIONS.md` here is empty.
